...
void estagio_MEM(typ_stt *estado)
{
    ...
    estado->MEM_WB.instrucao = estado->EX_MEM.instrucao; //propafgacao do EX
    estado->MEM_WB.valido = true;
    
    
    estado->MEM_WB.saida_mem = mem_data(
        estado->mem_dados,
        (unsigned int)estado->EX_MEM.resultado_ula,
        estado->EX_MEM.valor_b,
        estado->EX_MEM.sinal[esc_mem],
        !estado->EX_MEM.sinal[mem_reg], // Invertido: 0 vira 1 (Ativa Load)
        false 
    );
